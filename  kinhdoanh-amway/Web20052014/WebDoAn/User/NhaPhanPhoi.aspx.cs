...
public partial class User_NhaPhanPhoi : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (IsPostBack == false)
        {
            ...
            droCapDo.DataSource = cd.DS(cd.MaCD);
            droCapDo.DataBind();
    ...
    protected void griNhaPhanPhoi_SelectedIndexChanged(object sender, EventArgs e)
    {
       ...
       /* string NgayHienTai = System.DateTime.Now.ToLongTimeString();
        if(DATEDIFF (DATE, npp.NgayHetHan, NgayHienTai) <= 5)
            btnGHT.Visible = true;*/
        //if ((System.DateTime.Now.ToShortDateString() - npp.NgayHetHan) <= 5)
        //Tinh
