...
namespace webdoan
{
    ...
    public class NhaPhanPhoi
        {
            ...
            public DataTable TimNPP()//su dung ky thuat lay danh sach co tham so truyen vao
            {
                SqlConnection BaoVe = new SqlConnection("server=(local)\\SQLEXPRESS;uid=sa;pwd=123456;database=DoAn");
    ...
    public class ChamSoc
    {
        ...
        {
            string SelectSQL = "SELECT MaCS, ThoiGian, SoLan,ThamDu, CS.MaCT, TenCT, CS.MaKH, HoKH +' '+TenKH AS HoTenKH FROM ChamSoc CS, KhachHang KH, ChuongTrinh CT WHERE CS.MaNPP = @MaNPP AND CS.MaKH = KH.MaKH AND CS.MaCT = CT.MaCT";
            SqlConnection BaoVe = new SqlConnection("server=(local)\\SQLEXPRESS;uid=sa;pwd=123456;database=DoAn");
            DataTable ThungChua = new DataTable();
            BaoVe.Open();
            if (BaoVe.State == ConnectionState.Open)
