...
public partial class User_SanPham : System.Web.UI.Page
{
    ...
    protected void btnSuDung_Click(object sender, EventArgs e)
    {
        ...
        {
            npp.MaMH = int.Parse(griMatHang.SelectedValue.ToString());
            npp.MaNPP = Session["MaNPP"].ToString();
            npp.SoLuong = int.Parse(txtSoLuong.Text);
        ...
        {
            kh.MaMH = int.Parse(griMatHang.SelectedValue.ToString());
            kh.MaKH = Session["MaKH"].ToString();
            kh.MaNPP = Session["MaNPP"].ToString();
